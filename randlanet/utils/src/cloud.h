// Derived from HuguesTHOMAS/KPConv.
// Copyright (c) 2019 HuguesTHOMAS.
// SPDX-License-Identifier: MIT
// See KPCONV_LICENSE in this directory.
//
//
//      0==========================0
//      |    Local feature test    |
...
template <typename scalar_t> struct PointCloud {
    void set(const vector<scalar_t>& new_pts) {
        pts = new_pts.data();
        length = new_pts.size() / 3;
    }
    void set_batch(const vector<scalar_t>& new_pts, int begin, int end) {
        pts = new_pts.data();
        int start = begin * 3;
        pts += start;
        length = (end - begin);
    ...
    array<scalar_t, 3> operator[](const size_t index) const {
...
inline ostream& operator<<(std::ostream& os, const PointCloud<scalar_t>& P) {
